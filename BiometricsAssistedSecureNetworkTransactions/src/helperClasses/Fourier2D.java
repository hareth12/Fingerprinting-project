...
/**
this class provides the methods for the 2D-Fourier transformation
*/
...
public class Fourier2D {
	...
	public Complex[][] dft (Complex[][] G) {
		...
		for (int u = 0; u < M_2P; u++) {
			for (int v = 0; v < N_2P; v++) {
				GF[u][v] = new Complex();
				//ComputeThread[] computeThreads = new ComputeThread[M_2P];
				for (int m = 0; m < M_2P; m++) {
					//computeThreads[m] = new ComputeThread(gfz, G, m, v, GF, u);
					//computeThreads[m].run();
					gfz.real = 0.0; gfz.imag = 0.0;
					for (int n = 0; n < N_2P; n++) {
						gfz.add(Complex.mult(G[m][n], Complex.expi(-2.0*Math.PI*n*v/N_2P)));
					}	       
					(GF[u][v]).add(gfz.mult(Complex.expi(-2.0*Math.PI*m*u/M_2P)));
				}
				/*for(ComputeThread ct : computeThreads)
				{
					try {
						ct.join();
	...
	class ComputeThread extends Thread {
        ...
        Complex GF[][];
        int u;
        ComputeThread(Complex gfz, Complex[][]G, int m, int v, Complex GF[][], int u) {
            this.gfz = gfz;
            this.G = G;
            this.m = m;
            this.v = v;
